...
//go:embed route_template.go.tmpl
var templates embed.FS

// Route holds the template data
type Route struct {
	RouteName string
	FileName  string
}
...
func CreateRoute(routeName string) {
	...
	fileName := toSnakeCase(routeName)

	// Define the template file path (relative to the embedded filesystem)
	tmplFile := "route_template.go.tmpl" // File name for the embedded template

	// Get output directory from the environment variable
	outputDir := os.Getenv("PATH_FOR_ROUTE")
	if outputDir == "" {
		log.Fatalf("the path is not specified in env, please initialize the PATH_FOR_ROUTE variable")
	}
